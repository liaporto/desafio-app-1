...
describe('Edit data form', () => {
  ...
  const mockGetData: jest.Mock = jest.fn(async () => ({
    cpf: '222.222.222-22',
    pis: '222.2222.222-2',
    name: 'Teste',
  ...
  describe('render flow', () => {
    ...
    test('renders personal data inputs with data', async () => {
      ...
      const pisInput = screen.getByRole('textbox', {
        name: /pis/i,
      }) as HTMLInputElement;
    ...
    test('renders address inputs', () => {
      ...
      expect(screen.getByRole('textbox', { name: /rua/i })).toBeInTheDocument();
  ...
  describe('submit flow', () => {
    ...
    test('form shows error if required values are left empty', async () => {
      ...
      expect(
        screen.getByText('Email não pode ficar vazio')
      ).toBeInTheDocument();
      expect(
        screen.getByText('A senha não pode ficar vazia')
      ).toBeInTheDocument();
      expect(
        screen.getByText('A confirmação de senha não pode ficar vazia')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Município não pode ficar vazio')
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText('Número não pode ficar vazio')
      ).toBeInTheDocument();
    });
    ...
    test('form shows error if confirmPassword is not the same as password', async () => {
      ...
      expect(
        screen.getByText('As senhas não estão iguais')
      ).toBeInTheDocument();
    });
